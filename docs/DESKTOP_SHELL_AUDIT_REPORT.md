...
- **preload.mjs**: يكشف `electronAPI.setSize` لكن **لا قنوات `minimize/maximize/close/startDrag`** (مطلوبة لشريط عنوان مخصص). `window:auto-resize` بلا handler (قناة ميتة).
...
- لا شريط حالة حقيقي (رغم توفّر `window.api.backend.onStatusChanged` في `preload.mjs:127`).
...
**0. إطار النافذة** — `main.js:175`: أضف `frame:false` (أو `titleBarStyle:'hidden'` + `titleBarOverlay` كأبسط مسار). في `preload.mjs`: قنوات `windowControls:{minimize,maximize,close,isMaximized,onMaximizeChange}` + `ipcMain.handle` مقابلة في `main.js` (أعد استخدام تأكيد الإغلاق الموجود `main.js:281-312`).
