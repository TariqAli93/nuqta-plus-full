 ...
 * «خصم الفاتورة» across the invoice lines BEFORE profit is computed.
 *
 * Rules (mirrored 1:1 on the frontend in src/utils/discountAllocation.js):
 ...
 *    applied without selling at a loss is reported back so the UI can warn.
 *  - The minimum profit a line can reach because of the invoice discount is 0
 *    (never negative). Any pre-existing negative margin from an aggressive
 *    per-line discount is the user's own choice and is left untouched.
 *
 ...
 * @returns {{
 *   allocations:number[], // discount applied per line (post cost-floor)
 *   applied:number,       // total discount actually applied
 *   unapplied:number,     // requested − applied (blocked by the cost floor)
 *   capped:boolean,       // true when some discount could not be applied
 *   totalCapacity:number, // Σ max(0, value − cost) — the max safe discount
 * }}
 */
const EPS = 1e-6;
 ...
 * This MUST run before the invoice discount is distributed, so the invoice
 * discount is spread over the already-safe line nets.
 *
 ...
 *   per-unit figures (applied ≤ max = unitPrice − unitCost, never negative).
 */
...
export function allocateInvoiceDiscount(lines, requestedDiscount) {
  ...
  // remainder among the lines that are not yet saturated. Converges in ≤ n
  // rounds (each round saturates at least one line or finishes).
  ...
  for (let guard = 0; guard <= n && remaining > EPS; guard++) {
    ...
    if (!newlySaturated) {
      remaining = 0; // fully distributed this round
      break;
    }
  }
