...
const EMPTY_CAPABILITIES = Object.freeze({
  ...
  canUseBankAccounts: false,
  canUseSuppliers: false,
  canUsePurchases: false,
  canUseGL: false,
  canPostManualJournal: false,
  canUseAgentPricing: false,
...
export async function getUserCapabilities(user) {
  ...
  // Accounting-suite flags default OFF, so check `=== true` (the legacy flags
  // above default ON and use `!== false`).
  const treasuryOn = flags.treasury === true;
  const banksOn = flags.bankAccounts === true;
  const suppliersOn = flags.suppliers === true;
  const purchasesOn = flags.purchases === true;
  const glOn = flags.generalLedger === true;
  const manualJournalOn = flags.manualJournal === true;
  const agentPricingOn = flags.agentPricing === true;
  const financialReportsOn = flags.financialReports === true;
  ...
  return {
    ...
    // ── Accounting suite (treasury / purchases / GL / pricing) ──────────────
    // Receipt vouchers are cashier-level (cashiers collect debts today);
    // everything else money-sensitive starts at manager.
    canUseTreasury: treasuryOn && (isGlobal || isBA || isBM || isManager || isCashier),
    canManageTreasury: treasuryOn && (isGlobal || isBA),
    canUseBankAccounts: treasuryOn && banksOn && (isGlobal || isBA || isBM || isManager),
    canUseSuppliers: suppliersOn && (isGlobal || isBA || isBM || isManager),
    canUsePurchases: purchasesOn && (isGlobal || isBA || isBM || isManager),
    canUseGL: glOn && (isGlobal || isBA || isBM || isManager),
    canPostManualJournal: glOn && manualJournalOn && (isGlobal || isBA),
    canUseAgentPricing: agentPricingOn && (isGlobal || isBA || isBM || isManager || isCashier),
    // Financial statements derive from GL, so both flags must be on.
    canViewFinancialReports: financialReportsOn && glOn && (isGlobal || isBA || isBM || isManager),
  };
}
