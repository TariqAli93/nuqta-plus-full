...
/**
 * Resolve the unit price for a customer tier (تسعير الوكلاء). Pure function so
 * it can be unit-tested and shared. Resolution chain (first non-null wins):
 ...
 *   3. unit.salePrice
 *   4. product.sellingPrice
 *
 * `customerType` is 'retail' | 'wholesale' | 'agent'. Retail (or unknown) skips
 * the tier steps and falls straight to salePrice/sellingPrice.
 *
 * @param {{customerType?: string, unit?: object|null, product?: object}} args
 * @returns {number}
 */
export function resolveTierPrice({ customerType = 'retail', unit = null, product = {} } = {}) {
  ...
  const unitSale = unit ? num(unit.salePrice) : null;
  if (unitSale != null) return unitSale;
  return num(product?.sellingPrice) ?? 0;
}
...
function shapeUnitRow(row) {
  ...
  return {
    ...
    agentPrice: row.agentPrice == null ? null : Number(row.agentPrice),
...
export async function replaceProductUnits(tx, productId, units) {
  ...
  for (const u of resolvedInputs) {
    ...
    const values = {
      ...
      wholesalePrice:
        u.wholesalePrice === null || u.wholesalePrice === undefined || u.wholesalePrice === ''
          ? null
          : String(Number(u.wholesalePrice)),
      agentPrice:
        u.agentPrice === null || u.agentPrice === undefined || u.agentPrice === ''
          ? null
          : String(Number(u.agentPrice)),
