...
export const useAuthStore = defineStore('auth', {
  state: () => ({
    ...
    featureFlags: {},
    setupMode: 'done',
    // Operating mode (نمط العمل): 'simple' (محال صغيرة) أو 'full' (شركات).
    // Server-resolved; drives the upgrade CTA and full-only flag locks.
    appMode: 'simple',
  ...
  getters: {
    ...
    /** النمط الكامل — unlocks the advanced accounting modules. */
    isFullMode: (state) => state.appMode === 'full',
    /** النمط السهل — advanced accounting modules hidden + locked. */
  ...
  actions: {
    ...
    _hydrateFromSession(payload) {
      ...
      this.appMode = payload.appMode === 'full' ? 'full' : 'simple';
    ...
    async getProfile() {
      ...
      try {
        ...
        if (response.data) {
          const { scope, featureFlags, setupMode, appMode, capabilities, ...userOnly } =
            response.data;
          this._hydrateFromSession({
            user: userOnly,
            scope,
