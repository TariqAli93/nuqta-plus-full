...
export class ExpensesService {
  ...
  async create(input, user) {
    ...
    return await withTransaction(async (tx) => {
      ...
      // Treasury: mint the payment voucher (سند صرف) and stamp the expense
      // with its treasury attribution. No-op when the treasury flag is off.
      const voucher = await voucherService.mintForExpense(tx, {
        expense: row,
        user,
      ...
      // GL: post the expense entry (failure-valved).
      ...
      return { ...result, amount: n(result.amount) };
    });
  }
  ...
  async update(id, input, actingUser = null) {
    ...
    return await withTransaction(async (tx) => {
      ...
      // Keep the minted voucher's money fields in sync so the cashbox
      // balance always mirrors the expense.
      if (row.voucherId) {
        await tx
          ...
          .set({
            ...
            description: row.note ? `مصروف: ${row.note}` : `مصروف (${row.category})`,
          })
          .where(eq(schema.vouchers.id, row.voucherId));
      }

      // GL: amounts/category may have changed — reverse the old entry and
      // re-post from the fresh row (slot freed by the reversal).
      await glPostingService.reverseEntry(tx, {
        sourceType: 'expense',
        sourceId: row.id,
        user: actingUser,
        reason: 'تعديل المصروف',
      });
      await glPostingService.postDocument(tx, {
        sourceType: 'expense',
        sourceId: row.id,
  ...
  async delete(id, actingUser = null) {
    ...
    await withTransaction(async (tx) => {
      ...
      await voucherService.cancelForSource(tx, {
        expenseId: Number(id),
        reason: 'حذف المصروف الأصلي',
        userId: actingUser?.id || null,
      });
      // GL: reverse the expense entry before the row disappears.
      await glPostingService.reverseEntry(tx, {
        sourceType: 'expense',
        sourceId: Number(id),
        user: actingUser,
      ...
      await tx.delete(expenses).where(eq(expenses.id, Number(id)));
    });
